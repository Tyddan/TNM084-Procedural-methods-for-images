    ...
    while (!glfwWindowShouldClose(window))
    {
	    ...
	    for(i=0; i<IMAGE_SIZE; i++)
		{
		    ...
		    for(j=0; j<IMAGE_SIZE; j++)
			{
				...
				// Perlin noise
				//red = 90; // + 127*noise3(32.0*x, 32.0*y, 0.2*time);

				// Cellular (Worley) noise
				point[0] = 2.0*(x+(i+1/IMAGE_SIZE));
				point[1] = 2.0*(y+(j+1/IMAGE_SIZE));
				point[2] = 0.2*time;
				...
				red = 90 + 100*noise3(4.0*x- time*0.1, 12.0*y , 0.2*time);
				grn = 128 + 127*noise3(2.0*x- time*0.1, 10.0*y , 0.2*time);
				blu = 90 + 127*noise3(1.0*x, 1.0*y , 0.0*time);
				/*
				if (grn < 130 )
				{
					grn = 0.2*white;
